...
buildEntry = function(entry) {
    if (entry.poster_path === null) entry.poster_path = defaultPoster;
    var template = ['<div data-id=', entry.id, ' class="col-md-4 portfolio-item">', '<img class="img-responsive" src="http://image.tmdb.org/t/p/w500', entry.poster_path, '?api_key=2a6fd2d3356476f3bf594deb013e4f76" alt="">', '<h3>', '<a>' + entry.original_name + '</a>', '</h3>', '</div>'].join('');
    var $element = $(template);
    $element.data('item', entry);
    return $element;
};
buildDescription = function(entry) {
    return $(['<div class="', selectors.descriptionSelector, '">', '<p> Promedio Votos: ', entry.vote_average, '</p>', '<p> Número Votos: ', entry.vote_count, '</p>', '</div>'].join(''));
};
...
infiniteHandler = function(result) {
    ...
    if (result.total_results === 0) {
        $(selectors.searchLabel).html('No se encontraron resultados para: ' + $(selectors.searchLabel).html());
        return;
    }
...
drawResults = function(results) {
    ...
    if (toDraw.length === 0)
        if (page < pages) $.getJSON(request + '&page=' + (page + 1)).done(infiniteHandler);
...
buildChapterList = function(season) {
    var i, result = [];
    for (i = 0; i < season.episodes.length; i++) result.push('<li data-id=' + i + ' class="list-group-item">' + season.episodes[i].name + '</li>');
    return '<ul class="list-group">' + result.join('') + '</ul>';
};
fillPage = function(serie,
    season) {
    /*
...
buildPagination = function(seasons) {
    var i, results = [];
    for (i = 1; i <= seasons; i++) results.push('<li data-id="' + i + '"><a>' + i + '</a></li>');
    return results.join('');
};
...
$(document).ready(function() {
    ...
    $(selectors.parent).on('dblclick',
        delegate('.movies .portfolio-item img',
            clickHandler));
    $(selectors.modalFull).on('click',
        delegate('.modal-body .pagination a',
            paginationHandler));
    $(window).on('scroll',
        scrollHandler);
    /*
